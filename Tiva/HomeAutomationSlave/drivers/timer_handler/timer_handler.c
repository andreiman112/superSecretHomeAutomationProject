...
{ 
 ...
 if(TimerIntStatus(TIMER4_BASE,false))
	{
...
void TIMER3A_Handler(void)  //  Cyclic_1000ms
{ 
...
void TIMER2A_Handler(void)  //  Cyclic_1000ms
{ 
		TimerIntClear(TIMER2_BASE, TIMER_A);
    Cyclic_500ms();
   	TimerEnable(TIMER2_BASE, TIMER_A); 
}
void TIMER1A_Handler(void)  //Timer 2 A for Cyclic_100ms
{ 	
...
void TIMER0A_Handler(void)  //Cyclic_50ms
{ 
...
unsigned long getPeriphTimerAddress(unsigned long port)
{
    ...
    if (index <= (0x5))
        retAdr = SYSCTL_PERIPH_TIMER0 + index; //T0,T1,T2,T3,T4,T5
    else if (index == 6 || index == 7)
        retAdr = SYSCTL_PERIPH_WTIMER0 + (index - 6); //WIDET0, 	WIDET1
    else if (index >= (0xC) && index <= (0xF))
        retAdr = SYSCTL_PERIPH_WTIMER2 + (index - 0xc); //WIDET2,WIDET3,WIDET4,WIDET5
...
char getINTTIMER_Address(unsigned long base, unsigned long timer)
{
    ...
    char timerType = ((0x8000) & timer) >> 15;
    ...
    timerNumber = timerNumber >> 12;
    if (timerNumber <= 2)
        intRet = 35 + timerNumber * 2 + timerType;
    else if (timerNumber == 3)
        intRet = 51 + timerType;
    else if (timerNumber == 4)
        intRet = 86 + timerType;
    else if (timerNumber >= 5)
        intRet = 108 + (timerNumber - 5) * 2 + timerType;
    return intRet;
}
...
unsigned long Get_Timer_Config(unsigned long timer)
{
...
void Init_Timer(unsigned long base, unsigned long timer, unsigned long period) //problems with TimerB
{
	...
	if(timer&0xff && !(timer&0xff00))//Timer A
	{
		config=TIMER_CFG_A_PERIODIC;
		time_out=TIMER_TIMA_TIMEOUT;
	}
	else if(!(timer&0xff) && (timer&0xff00))//Timer B
	{
		config=TIMER_CFG_B_PERIODIC;
		time_out=TIMER_TIMB_TIMEOUT;
	}
    ...
    TimerIntDisable(base, time_out);

    TimerDisable(base, timer);
    TimerClockSourceSet(base, TIMER_CLOCK_SYSTEM);
    TimerConfigure(base, config);

    TimerLoadSet(base, timer, TIMER_reload_calculator(period));

    TimerIntEnable(base, time_out);
